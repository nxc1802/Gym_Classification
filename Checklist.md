...
- [x] **5. Sửa latency wording (Classifier Latency ≠ End-to-End Latency)**
  ...
  - **Metrics**:
    - **Classifier inference latency**: $0.42\text{--}4.33\text{ ms}$ on host CPU ($0.08\text{--}0.54\text{ ms}$ on CUDA, $1.11\text{--}8.77\text{ ms}$ synchronized MPS; single backbones $0.42\text{--}1.16\text{ ms}$ CPU, SkelGym-Lite $1.44\text{ ms}$ CPU, SkelGym-Full $4.33\text{ ms}$ CPU).
  ...
  - **Standardization**: Replaced un-synchronized early baseline with rigorous synchronized benchmarks across CPU ($0.42\text{--}4.33\text{ ms}$), MPS ($1.11\text{--}8.77\text{ ms}$), and CUDA ($0.08\text{--}0.54\text{ ms}$).
